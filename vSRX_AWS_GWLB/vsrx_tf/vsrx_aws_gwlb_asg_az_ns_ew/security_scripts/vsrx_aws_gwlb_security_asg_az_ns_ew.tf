...
resource aws_lambda_function "vsrx_asg_lambda_r" {
  ...
  vpc_config {
    ...
    subnet_ids         = [aws_subnet.data_subnet1_r.id,
                          aws_subnet.data_subnet2_r.id]
  }
  runtime      = "python3.9"
  environment {
    variables = {
...
resource "aws_autoscaling_group" "vsrx_asg_r" {
  ...
  vpc_zone_identifier      = [aws_subnet.data_subnet1_r.id,
                               aws_subnet.data_subnet2_r.id]
  ...
  initial_lifecycle_hook {
    ...
    notification_metadata = <<EOF
    {
      ...
      'data_idx_at': '0',
      'snet_info': {
          '${var.availability_zones_p[0]}':{
            'mgmt': {'snet_id':'${aws_subnet.mgmt_subnet1_r.id}', 'sg':'${aws_security_group.vsrx_sg_mgmt_r.id}', 'idx':'1', 'alloc_eip':'true'}
          },
          '${var.availability_zones_p[1]}':{
            'mgmt': {'snet_id':'${aws_subnet.mgmt_subnet2_r.id}', 'sg':'${aws_security_group.vsrx_sg_mgmt_r.id}', 'idx':'1', 'alloc_eip':'true'}
          }
        }
    }
    EOF
